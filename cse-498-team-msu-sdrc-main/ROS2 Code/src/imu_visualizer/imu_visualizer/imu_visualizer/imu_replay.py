...
# Not used in our current implementation. But will be useful on systems that support the camera IMU

class imuReplay(Node):
    """Class for an ROS node that replays the IMU data"""
    def __init__(self,bagname):
        ...
        storage_options= rosbag2_py.StorageOptions(
            uri = os.path.join('$HOME/rosbag_data/imu_rosbags/', bagname),
            storage_id='sqlite3'
        )
...
def main(args=None):
    """Start the node"""
    if len(sys.argv) != 2:
        print("please provide a single folder name (relative to $HOME/rosbag_data/imu_rosbags/ )")
    else:
